...
public class TSVDatasetMetrics
{
	...
	public static class Options
	{
		...
		public boolean cohensKappa = true;
	...
	private void makeOutput( final List<Map<String, ScoredAnnotation<String>>> x )
	{
		...
		{
			if( x.size() < 2 )
				System.out.println( "Cohen's Kappa requires two datasets - one for each rater");
			else
			{
				if( x.size() > 2 )
					System.out.println( "WARNING: Using only the first two datasets for Cohen's Kappa" );
		...
		if( this.options.precision )
		{
			...
			else
			{
				...
				if( missingInTestSet > 0 )
					System.out.println( "Missing from test set: "+missingInTestSet );
			}
		}
	}
