...
public class DeltaSearcherTool {
	...
	public static void main(String[] args) throws IOException, SearcherException, ParserConfigurationException,
			SAXException
	{
	...
	private static void search(File index, String query, File shotsDirCacheFile, File graphFile) throws IOException,
			SearcherException
	{
		final Directory indexDir = FSDirectory.open(index);
		final IndexReader indexReader = DirectoryReader.open(indexDir);

		final LSHDataExplorer lshExplorer = new LSHDataExplorer(graphFile, 10);

		final DeltaSearcher deltaSearcher = new DeltaSearcher("DeltaSearcher", indexReader, shotsDirCacheFile,
				lshExplorer);

		final Query q = new Query("CLI", query, null);
			...
			SAXException
	{
		...
		final SearcherEvaluator eval = new SearcherEvaluator(deltaSearcher);

		final Map<Query, List<Result>> expectedResults =
				SearcherEvaluator.importExpected(queriesFile, resultsFile);

		final Float[] initial = { 0f, 1.5f, 2f, 1f, 1f, 0f, 0f, 60 * 1f, 60 * 10f, 0f, 0f, 0f };
		final Float[] increment = { 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 0.1f, 0.1f, 0.1f };
		final Float[] termination = { 0f, 1.5f, 2f, 1f, 1f, 0f, 0f, 60 * 1f, 60 * 10f, 1f, 3f, 3f };

		final Vector evaluation = eval.evaluateAgainstExpectedResults(expectedResults,
				WINDOW);/*
						 * initial, increment, termination);
						 */
		/*
		 * Vector evaluation =
