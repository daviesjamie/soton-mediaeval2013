...
public class SWTTextDetector {
	...
	public static List<WordCandidate> findWords(FImage image) {
		image = image.normalise();
		FImage swt = image.process(new StrokeWidthTransform(true, 2));
		List<ConnectedComponent> comps = findComponents(swt);
		List<LetterCandidate> letters = filterComponents(comps, swt, image);
		List<LineCandidate> lines = LineCandidate.extractLines(letters);

		final List<WordCandidate> words = new ArrayList<WordCandidate>();
		for (final LineCandidate line : lines) {
			words.addAll(WordCandidate.extractWords(line));
		}

		image = image.inverse();
		swt = image.process(new StrokeWidthTransform(true, 2));
		comps = findComponents(swt);
		letters = filterComponents(comps, swt, image);
		lines = LineCandidate.extractLines(letters);
