...
public class BasicCEDDEngine extends MeanShiftTagEngine {
			...
			double bandwidth) throws IOException
	{
		...
		this.visualSearcher = new InMemCEDDPQSearcher(ceddData, searcher);
		this.numResults = numResults;
	}

	protected List<GeoLocation> search(long query) {
		try {
			final ScoreDoc[] hits = visualSearcher.search(query, numResults);
	...
	public GeoLocationEstimate estimateLocation(QueryImageData query) {
		...
		if (pts.size() == 0) {
			System.err.println("Image " + query.flickrId + " had 0 results");
