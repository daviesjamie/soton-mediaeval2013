...
public class TimeOfDayDelta implements SimMatProvider {
	double base;

	public TimeOfDayDelta(double base) {
		this.base = base;
	}
	...
	public SparseMatrix computeSimilarityMatrix(List<ObjectDoublePair<ResultItem>> input) {
		...
		for (int i = 0; i < input.size(); i++) {
			...
			for (int j = i + 1; j < input.size(); j++) {
				...
				final int mi = ri.date_taken.getHours();
				final int mj = rj.date_taken.getHours();
