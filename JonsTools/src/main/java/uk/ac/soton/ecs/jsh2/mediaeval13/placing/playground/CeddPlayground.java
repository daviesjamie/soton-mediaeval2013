...
import uk.ac.soton.ecs.jsh2.mediaeval13.placing.search.InMemCEDDKDTreeSearcher;

/**
 * Demonstrate simple CEDD search
 * 
...
public class CeddPlayground {
	public static void main(String[] args) throws IOException {
		...
		final InMemCEDDKDTreeSearcher ceddSearcher = new InMemCEDDKDTreeSearcher(ceddData, luceneSearcher);
		...
		while ((line = br.readLine()) != null) {
			...
			try {
				...
				results = ceddSearcher.search(query, numResults);
			} catch (final NumberFormatException e) {
				try {
					final URL u = new URL(line);
					results = ceddSearcher.search(ImageUtilities.readMBF(u), numResults);
