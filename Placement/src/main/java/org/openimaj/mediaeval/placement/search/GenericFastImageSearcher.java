...
public class GenericFastImageSearcher extends AbstractImageSearcher {
	...
	public GenericFastImageSearcher(int maxHits, Class<?> descriptorClass, String fieldName) {
		...
		try {
			...
			logger.log(
					Level.SEVERE,
					"Error instantiating class for generic image searcher (" + descriptorClass.getName() + "): "
							+ e.getMessage());
	...
	public GenericFastImageSearcher(int maxHits, Class<?> descriptorClass, String fieldName, boolean isCaching,
			IndexReader reader) {
	...
	protected float findSimilar(IndexReader reader, LireFeature lireFeature) throws IOException {
		...
		if (!isCaching) {
			// Read each and every document from the index and then compare it
			// to the query
			for (int i = 0; i < docs; i++) {
				...
				// If this document is nearer to the query than at least one of
				// the current set
			...
			for (Iterator<byte[]> iterator = featureCache.iterator(); iterator.hasNext();) {
				...
				if (reader.hasDeletions() && !liveDocs.get(count)) {
					...
					// If this document is nearer to the query than at least one
					// of the current set
					else if (tmpDistance < maxDistance) {
						// Remote the last one
						this.docs.remove(this.docs.last());
	...
	protected float getDistance(Document document, LireFeature lireFeature) {
		if (document.getBinaryValue(fieldName) != null && document.getBinaryValue(fieldName).length > 0) {
			cachedInstance.setByteArrayRepresentation(document.getBinaryValue(fieldName).bytes,
					document.getBinaryValue(fieldName).offset, document.getBinaryValue(fieldName).length);
			return lireFeature.getDistance(cachedInstance);
		} else {
			logger.warning("No " + descriptorClass.getName() + " feature stored for "
					+ document.get(LuceneIndexBuilder.FIELD_ID));
		}
