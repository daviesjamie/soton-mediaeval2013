...
public class LireSearcher implements VisualSearcher {
	...
	public LireSearcher(String lireIndex, LireFeatures feature, IndexSearcher metaSearch, int maxHits)
			throws IOException {
	...
	public ScoreDoc[] search(long flickrId, int numResults) throws IOException {
		...
		try {
			...
			// final Query q = new QueryParser( Version.LUCENE_43,
			// LuceneIndexBuilder.FIELD_ID, new StandardAnalyzer(
			// Version.LUCENE_43 ) ).parse( String.valueOf( flickrId ) );
