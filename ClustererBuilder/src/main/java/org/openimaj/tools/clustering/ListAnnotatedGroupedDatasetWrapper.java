...
public class ListAnnotatedGroupedDatasetWrapper<OBJECT, ANNOTATION>
	...
	public Iterator<Annotated<OBJECT, ANNOTATION>> iterator() {
		class AnnotatedIterator<OBJECT, ANNOTATION> implements Iterator<Annotated<OBJECT, ANNOTATION>> {
		...
		/* JLP Sexy Recursion Remix
		return new Iterator<Annotated<OBJECT, ANNOTATION>>(dataset) {
			Iterator<ANNOTATION> currentGroup = dataset.keySet().iterator();
			Iterator<OBJECT> currentObject = currentGroup.hasNext() ? dataset.get(currentGroup.next()).iterator() : null;
			...
			public boolean hasNext() {
				...
				if (currentGroup.hasNext()) {
					currentObject = dataset.get(currentGroup.next()).iterator();
					return hasNext();
				}
