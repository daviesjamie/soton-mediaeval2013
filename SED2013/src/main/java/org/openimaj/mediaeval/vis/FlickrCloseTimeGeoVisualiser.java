 ...
 *
 */
public class FlickrCloseTimeGeoVisualiser {

	private Logger logger = Logger.getLogger(FlickrCloseTimeGeoVisualiser.class);
	 ...
	 * @throws IOException 
	 */
	public FlickrCloseTimeGeoVisualiser(Training photos, Date start, Date end) throws IOException {
		final WorldMap<LabelledDot> wp = new WorldMap<LabelledDot>(1200,600, new LabelledPointVisualisation());
		wp.setDefaultCountryLandColour(RGBColour.BLACK);
		wp.setDefaultCountryOutlineColour(RGBColour.WHITE);
...
//			clusterColourMap.put(cluster, ColourMap.Hot.apply(i++/(float)photos.size()));
		...
		for (Photo photo : photos) {
			...
			wp.addPoint(x, y, new LabelledDot("", 1, clusterColours[index]));
		}
		...
		wp.showWindow("Close time world map");
		ImageUtilities.write(wp.getVisualisationImage(),new File("/Users/ss/Experiments/sed2013/worldmapvis.png"));
		ImageUtilities.write(vis.getVisualisationImage(),new File("/Users/ss/Experiments/sed2013/timevis.png"));
	...
	public static void main(String[] args) throws IOException, SAXException, ParserConfigurationException, ParseException, XMLStreamException {
		...
		FlickrCloseTimeGeoVisualiser fpwv = new FlickrCloseTimeGeoVisualiser(dataset,start,end);
	}
